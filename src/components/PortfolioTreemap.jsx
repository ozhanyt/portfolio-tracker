    ...
    return (
        <g>
            <rect
                ...
                height={height}
                style={{
                    fill: payload.color, // Use color calculated in data prep
                    stroke: '#fff',
...
export function PortfolioTreemap({ data }) {
    ...
    // We colorize based on returnRate: >= 0 Green, < 0 Red
    const chartData = data
        .filter(item => item.value > 0) // Only show positive value items
        .map(item => {
            let color = '#9ca3af'; // Default gray
            if (item.returnRate > 0) color = '#10b981'; // Green-500
            else if (item.returnRate < 0) color = '#ef4444'; // Red-500

            // Adjust opacity/shade based on magnitude could be cool, but keep simple for now
            return {
                name: item.code,
                size: item.value, // Treemap size based on value
                ...
                <div className="h-[400px] w-full">
                    {chartData.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                            <Treemap
                                data={chartData} // Recharts Treemap takes array of items directly if no nesting? No, usually nested.
                                // Wait, simple flat array works for some versions, but standard is nested. 
                                // Actually, let's look at docs or standard usage. 
                                // If I pass flat array `chartData` directly as `data`, Recharts might treat them as roots?
                                // Let's try passing `chartData` (array of objects) directly.
                                dataKey="size"
                                aspectRatio={4 / 3}
                                stroke="#fff"
